...
/* The Spectrum's attribute clash (one ink+paper pair per 8x8 cell) makes
   per-character colour awkward for small UI glyphs, so every card is
   shown as a bracketed [label:COLORLETTER+VALUE] (same trick as the
   VIC-20/PET/Atari ports) instead. Selection uses INVERSE (ink/paper
   swap) instead of a colour change. Screen is 32 columns, narrower than
   every other port here, so layout spacing below is Spectrum-specific
   (5 cards/row instead of 6, tighter opponent/color-picker spacing). */
...
void ui_title_screen(void) {
    scr_clear();
    scr_puts(13, 1, "U N O", 0);
    scr_puts(4, 3, "FOR THE ZX SPECTRUM", 0);
    scr_puts(1, 6, "1 PLAYER VS 3 COMPUTERS", 0);
    scr_puts(1, 8, "O/P: PICK A CARD", 0);
    scr_puts(1, 9, "SPACE/ENTER: PLAY OR CONFIRM", 0);
    scr_puts(1, 10, "Q: DRAW A CARD", 0);
    scr_puts(0, 11, "OR PRESS 1-9,0,A-J FOR THAT CARD", 0);
    scr_puts(0, 14, "CARDS SHOW AS [LABEL:COLOR+VAL]:", 0);
    scr_puts(0, 15, "R=RED Y=YELLOW G=GREEN B=BLUE", 0);
    scr_puts(0, 16, "S=SKIP V=REVERSE D=DRAW2", 0);
    scr_puts(0, 17, "W=WILD F=WILD+4", 0);
    scr_puts(4, 20, "PRESS SPACE TO START", 0);
}
...
void ui_draw_color_picker(unsigned char selected) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        scr_put((unsigned char)(x + 1 + lens[i]), MSG_Y2, sel ? ']' : ' ', 0);
