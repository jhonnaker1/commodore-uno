/* The bare C64 port's SID effects (../c64/src/sid.c), with every register
   access made volatile. cc65 never optimises a register access away, so the
   original can leave it out; llvm-mos does not have to read $D012 twice.
   Compiled unmodified with llvm-mos -Oz, sfx_draw gates both voices on and
   then becomes `jmp .` -- the raster wait folded into an infinite loop, so
   the first card anyone drew would have hung the machine.

   Under C64 OS the SID and the VIC's raster counter are only visible with
   I/O banked in, so main.c brackets every effect with io_on()/io_off(). */
#include "sid.h"
...
#define SID_V3_PW_LO (*(volatile unsigned char *)0xD410)
#define SID_V3_PW_HI (*(volatile unsigned char *)0xD411)
...
#define SID_FC_LO (*(volatile unsigned char *)0xD415)
#define SID_FC_HI (*(volatile unsigned char *)0xD416)
#define SID_RES_FILT (*(volatile unsigned char *)0xD417)
...
void sid_init(void) {
    unsigned char i;
    for (i = 0; i < 25; i++) {
        *((volatile unsigned char *)0xD400 + i) = 0;
    }
