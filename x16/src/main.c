...
/* Glides a real VERA hardware sprite (a suit-colored card) between two
   hand/table cells -- the X16 has 128 hardware sprites, so unlike the
   VBXE port's redraw-based block this is genuine sprite motion, stepped
   smoothly in pixel space (spr_move takes cell coords, converts to pixels
   internally). `suit` is a COLOR_RED..COLOR_WILD index. The caller redraws
   the table/hand right after, so the sprite just hides itself at the end. */
#define TOSS_STEPS 16
static void animate_toss_to(unsigned char from_col, unsigned char from_row,
                             unsigned char to_col, unsigned char to_row, unsigned char suit) {
    unsigned char step;
    int x0 = from_col, y0 = from_row, x1 = to_col, y1 = to_row, dx = x1 - x0, dy = y1 - y0;

    spr_show(suit, from_col, from_row);
...
static void animate_toss(unsigned char from_col, unsigned char from_row, unsigned char suit) {
    unsigned char to_col, to_row;
    ui_top_card_pos(&to_col, &to_row);
    animate_toss_to(from_col, from_row, to_col, to_row, suit);
}
...
static void animate_deal(GameState *g) {
    ...
    for (i = 0; i < full_count; i++) {
        ...
        animate_toss_to(dcol, drow, col, row, COLOR_BLUE); /* blue card-back sprite */
        sfx_draw();
...
static unsigned char try_play(GameState *g, unsigned char idx, unsigned char *game_over) {
    ...
    sfx_card_play();
    animate_toss(col, row, (unsigned char)(c.color == COLOR_WILD ? chosen : c.color));
    wait_vsync();
    ui_draw_opponents(g);
    ui_draw_table(g);
    ui_draw_hand(g, 0);
    ...
    for (;;) {
        ...
        if (pressed & IN_UP) {
            ...
            if (is_legal(g, drawn)) {
                ...
                play_card(g, idx, chosen);
                sfx_card_play();
                animate_toss(col, row, (unsigned char)(drawn.color == COLOR_WILD ? chosen : drawn.color));
                wait_vsync();
                ui_draw_opponents(g);
                ui_draw_table(g);
                ui_draw_hand(g, 0);
