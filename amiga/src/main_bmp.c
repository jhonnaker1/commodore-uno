/* Full playable bitmap UNO for the Amiga. Same turn logic as the text
   build (main.c), but rendered through the graphics layer (gfx.c + ui_bmp.c)
   instead of console.device text. Redraw-on-change; selection is shown by
   the lifted/highlighted card in the fanned hand. */
#include <stdlib.h>
#include "gfx.h"
...
static unsigned char human_turn(GameState *g) {
    ...
    for (;;) {
        if (redraw) {
            ...
            if (human_has_legal_move(g)) ui_message("YOUR TURN", ", / . , SPACE, U, OR A KEY");
            else ui_message("YOUR TURN", "NO PLAYABLE CARD - U TO DRAW");
            redraw = 0;
        }
        wait_vsync();
        pressed = joy_pressed();
