...
/* Native-mode main loop. Identical in structure to main.c (the C64-mode
   build), with two differences:

   - No <ascii_charmap.h>. That include exists in the C64-mode sources only
     to stop cc65 applying its PETSCII charmap to string literals. llvm-mos
     has no such concept, so literals are already ASCII.

   - No hardware-sprite card toss. main.c glides a VIC-II sprite from the
     played card to the discard pile, using facts a probe confirmed about
     C64 mode: screen at $0400, VIC bank 0, sprite pointers at $07F8. None
     of those hold here -- native mode relocates screen RAM (the base comes
     from $D060-$D063) and H640 changes sprite pointer handling, which is
     why the MEGA65 has extended pointer registers at $D06C-$D06E in the
     first place. Rather than guess, the animation is left out of this
     first native build; adding it back wants the same probe-first approach
     that got it working in C64 mode. */
...
static unsigned char human_turn(GameState *g) {
    ...
    for (;;) {
        if (redraw) {
            ...
            ui_message("YOUR TURN",
                       "LEFT/RIGHT THEN FIRE, UP TO DRAW, OR PRESS A CARD'S KEY");
            redraw = 0;
        }
        wait_vsync();
        pressed = joy_pressed();
