/* Full playable bitmap UNO for the Atari VBXE. Same turn logic as the
   text-mode VBXE port (main_vbxe.c), but rendered through the bitmap
   ui_bmp.c / vbxebmp.c layer. No hardware-sprite toss or blink cursor here
   -- the graphics UI redraws on change and shows selection via the
   lifted/highlighted card in the fanned hand (mirrors the X16 bitmap
   build). */
#include <stdlib.h>
#include "vbxebmp.h"
...
static unsigned char human_turn(GameState *g) {
    ...
    for (;;) {
        if (redraw) {
            ...
            if (human_has_legal_move(g)) ui_message("YOUR TURN", "L/R, FIRE, UP, OR A KEY");
...
int main(void) {
    ...
    for (;;) {
        ui_title_screen();       /* enters graphics mode + draws title */
