/* Turn loop for the DOS graphics builds (EGA and VGA). Identical in flow to
   main.c, the CGA text build -- the only differences are that it brings up
   gfx.h's backend instead of the text layer, and that everything it draws
   goes through ui_bmp.c. Kept as a separate file rather than #ifdef'd into
   main.c, matching how amiga/, x16/ and atari/ carry their bitmap builds.
 */
#include <stdlib.h>
#include <dos.h>
#include "gfx.h"
