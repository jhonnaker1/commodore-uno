/* UNO for the Foenix F256K. Same turn logic as the other ports (shared
   cards.c/game.c/ai.c); this handles presentation timing and input. Renders
   colour-tile cards through the Vicky text mode (f256vid.c + ui_f256.c).
   No animation -- redraw-on-change, selection shown by the highlighted card
   in the hand. */
#include <stdlib.h>
#include "f256vid.h"
#include "f256snd.h"
...
static unsigned char human_turn(GameState *g) {
    ...
    for (;;) {
        if (redraw) {
            ...
            if (human_has_legal_move(g)) ui_message("YOUR TURN", ", . MOVE  SPACE PLAY  U DRAW");
            else ui_message("YOUR TURN", "NO PLAYABLE CARD - PRESS U TO DRAW");
            redraw = 0;
        }
        wait_vsync();
        pressed = joy_pressed();
