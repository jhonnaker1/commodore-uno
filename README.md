...
| [`mega65/`](mega65) | MEGA65 | Complete — the modern Commodore-65 recreation, in **two builds**. The C64-mode one (cc65, `$0801`) brings up the VIC-IV via [mega65-libc](https://github.com/mega65/mega65-libc) for a per-cell-color 40-column screen, a **VIC-II hardware-sprite** card toss on every play, and SID at `$D400`. The **native-mode** one (llvm-mos, `$2001` with a BASIC 65 header) runs in real C65/MEGA65 mode, where VIC-IV's H640 is genuinely available: **80 columns**, 5-wide card tiles with the value knocked out of a solid suit block, and an 11/9 hand split. (Legal-move dimming, stereo dual-SID, and a native-mode card toss are noted follow-ups — see [`mega65/`](mega65).) |
...
cd mega65 && make run XMEGA65=/path/to/xmega65 M65ROM=/path/to/mega65-rom.bin  # build/uno.prg in Xemu's xmega65 (C64 mode, 40 col)
cd mega65 && make run-native XMEGA65=/path/to/xmega65 M65ROM=/path/to/mega65-rom.bin  # build/uno-native.prg (native mode, 80 col)
...
- **MEGA65**: the only machine here built twice, because one binary cannot
  reach all of it. The C64-mode build compiles as a cc65 `c64`-target
  program that then unlocks the VIC-IV through
  [mega65-libc](https://github.com/mega65/mega65-libc) -- a C64 binary that
  stops being one at `mega65_init()`. The awkwardness is at link time:
  mega65-libc's `random.o` defines its own `rand`/`srand`, which collide
  with cc65's stdlib, and the shared game logic calls `rand()`. The Makefile
  drops `random.o` from the built `libmega65.a` rather than patching either
  side. Text goes through `cputcxy()`, which takes raw screen codes, so
  `mega65vid.c` translates PETSCII on the way out while the UI's string
  literals stay ASCII via `<ascii_charmap.h>`.

  What that build *cannot* do is 80 columns, and the reason is not the video
  chip. A `$0801` load address puts the machine in C64 mode, and the C64
  never had 80 columns; setting VIC-IV's H640 bit from in there changes
  nothing. Colour RAM makes it concrete -- 80x25 is 2000 cells, and a
  C64-mode program can only reach the 1K window at `$D800`. So the second
  build links to `$2001` with a BASIC 65 header, which is what actually
  selects native mode, and there H640 works and mega65-libc reaches the real
  colour RAM at `$FF80000` through the 45GS02's 32-bit addressing. cc65 has
  no MEGA65 target at all, so that build uses **llvm-mos** -- which, unlike
  the TMS9900, vbcc and Open Watcom toolchains here, ships prebuilt for
  macOS and needed no source build. It also generates markedly tighter code:
  11,316 bytes against cc65's 26,042 for the same game.

  Native mode simplifies as much as it unlocks. Keyboard input comes from
  `$D610`, which hands over **ASCII directly**, so `input_native.c` needs no
  PETSCII translation and no `<ascii_charmap.h>` anywhere -- llvm-mos has no
  charmap concept to begin with. The one thing that did not survive the move
  is the sprite card toss: it depends on C64-mode facts (screen at `$0400`,
  sprite pointers at `$07F8`) that native mode changes, which is why the
  MEGA65 has extended sprite pointer registers at `$D06C-$D06E` at all.
