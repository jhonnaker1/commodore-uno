...
/* Standard Apple II text mode has no per-character-cell color, so every
   card is shown as a bracketed [label:COLORLETTER+VALUE] (same trick
   as the VIC-20/PET/Atari ports) instead of relying on a color change.
   Selection is shown with reverse video instead, which conio's revers()
   does support per character on the Apple II. */
...
void ui_title_screen(void) {
    ...
    scr_puts(7, 4, "FOR THE APPLE IIE", 0);
    ...
    scr_puts(10, 22, "PRESS SPACE TO START", 0);
}
...
void ui_game_over_screen(unsigned char human_won, unsigned char winner_idx) {
    ...
    scr_puts(7, 20, "PRESS SPACE TO PLAY AGAIN", 0);
}
