...
void ui_title_screen(void) {
    ...
    scr_puts(6, 5, "FOR THE COMMODORE 128 (80-COL VDC)", COL_WHITE);
...
void ui_draw_opponents(GameState *g) {
    unsigned char opp, x;
    scr_fill_rect(0, OPP_Y, 40, 1, 32, COL_BLACK);
    for (opp = 1; opp <= 3; opp++) {
        x = 2 + (opp - 1) * 13;
...
void ui_draw_table(GameState *g) {
    ...
    scr_fill_rect(0, TABLE_INFO_Y, 40, 1, 32, COL_BLACK);
    ...
    scr_put(TOP_X + 1, TABLE_INFO_Y, CH_SOLID_BLOCK, suit_color[g->top_color]);
...
void ui_draw_hand(GameState *g, unsigned char cursor) {
    ...
    scr_fill_rect(0, HAND_LABEL_Y, 40, 1, 32, COL_BLACK);
    scr_puts(1, HAND_LABEL_Y, "YOUR HAND", COL_WHITE);
    scr_put_num(11, HAND_LABEL_Y, p->count, COL_WHITE);

    scr_fill_rect(0, HAND_Y, 40, 4, 32, COL_BLACK);
    scr_fill_rect(0, LABEL1_Y, 40, 1, 32, COL_BLACK);
    if (has_row2 || prev_had_row2) {
        scr_fill_rect(0, HAND2_Y, 40, 4, 32, COL_BLACK);
        scr_fill_rect(0, LABEL2_Y, 40, 1, 32, COL_BLACK);
    }
...
void ui_message(const char *line1, const char *line2) {
    scr_fill_rect(0, MSG_Y1, 40, 2, 32, COL_BLACK);
...
void ui_event_uno(unsigned char idx) {
    unsigned char x;
    scr_fill_rect(0, MSG_Y2, 40, 1, 32, COL_BLACK);
